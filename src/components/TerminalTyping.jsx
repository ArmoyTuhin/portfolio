...
const TerminalTyping = () => {
  ...
  const terminalLines = [
    ...
    { type: 'output', text: '> Dream: Being a great researcher, nomad, problem solver and solution maker' },
    { type: 'command', text: '$ Why me ?' },
    { type: 'output', text: '> I have always cravings for learning something new' },
    { type: 'output', text: 'tuhin@portfolio:~$' },
  ];
  ...
  return (
    <motion.div
      ...
      className="w-full"
    >
      <div className="bg-black/90 backdrop-blur-sm border-2 border-white/20 rounded-lg overflow-hidden shadow-2xl"
        style={{ boxShadow: '0 0 30px rgba(255, 255, 255, 0.1)' }}
      >
        {/* Terminal Header */}
        <div className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 px-3 md:px-4 py-2 md:py-3 flex items-center gap-1.5 md:gap-2 border-b border-white/10">
          <div className="flex gap-1.5 md:gap-2">
            <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-red-500"></div>
            <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-yellow-500"></div>
            <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-green-500"></div>
          </div>
          <span className="text-white text-xs md:text-sm font-mono ml-1 md:ml-2">tuhin@portfolio:~</span>
        </div>
        ...
        <div className="p-4 sm:p-5 md:p-6 font-mono text-xs sm:text-sm md:text-base min-h-[300px] sm:min-h-[360px] md:min-h-[420px]">
          ...
          <div className="flex justify-center mb-4 md:mb-6">
            <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-lg overflow-hidden border-2 border-white/20" style={{boxShadow: '0 0 20px rgba(255, 255, 255, 0.1)'}}>
              <img
                src={tuhinPortrait}
                alt="Tuhin Chowdhury"
